...
export default function Root() {
  ...
  const setFocus = (id) => {
    ...
    const index = updatedTabs.findIndex(element =>
      element.id === id
    );
    if (index === -1) {
      return;
    }
    updatedTabs[index].focus = true;
    setTabs(updatedTabs);
    setTabFocus(id);
    update(prevUpdater => !prevUpdater);
  };
  const removeFocus = () => {
    let updatedTabs = tabs.map(element => {
      let elementNoFocus = element;
      elementNoFocus.focus = false;
      return elementNoFocus;
    });
    setTabs(updatedTabs);
    update(prevUpdater => !prevUpdater);
  };
